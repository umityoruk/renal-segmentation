...
def dicom2hdf5(dicom_path, output_path):
    # dicom_list = []
    # for (dirpath, dirnames, filenames) in os.walk(dicom_path):
    #     for filename in filenames:
    #         if '.dcm' in filename.lower():
    #             dicom_list.append(os.path.join(dirpath, filename))
    ...
    for i in range(len(dicom_list)):
        ...
        img_list.append((ds.pixel_array, slice_location, trigger_time))
        # if slice_location == None:
        #     slice_location = ds.SliceLocation
        # elif slice_location == ds.SliceLocation:
        #     if ds.dir('TriggerTime'):
        #         temporal_resolution = int(ds.TriggerTime) # ms
        #     break
        # num_slices += 1
    print('\tDone')
    sys.stdout.flush()
    ...
    # num_timepoints = len(dicom_list)//num_slices
    ...
    # keys = [(x[1], x[2]) for x in img_list]
    # print(keys)
    # sys.stdout.flush()
    ...
    #         sys.stdout.flush()
    #     ds = dicom.read_file(dicom_list[i])
    #     data4d[:,:,i%num_slices,i//num_slices] = ds.pixel_array
    # print('\tDone')
    # sys.stdout.flush()
    
    # Save the data in a hdf5 file.
#     output_path = os.path.join(dicom_path, 'recon4d.hdf5')
    print('Saving output ...')
    sys.stdout.flush()
    with h5py.File(output_path, 'w') as f:
        ...
        dset8 = f.create_dataset('accession_number', data=accession_number)
    print('Done')
    sys.stdout.flush()
